...
def test_upgrade(device, device_user, device_password, device_host, app_archive_path, app_domain, app_dir, app):
    device.run_ssh('snap remove {0}'.format(app))
    device.run_ssh('snap install {0}'.format(app))
    local_install(device_host, device_password, app_archive_path)
    wait_for_rest(requests.session(), "https://{0}".format(app_domain), 401, 10)
